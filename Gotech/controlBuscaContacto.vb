Public Class controlBuscaContacto
    ...
    Sub llenaLista()
        ...
        If Buffer.pClientes = 1 And Buffer.pSeguimientos = 1 And Buffer.pProductos = 1 And Buffer.pVentasRealizar = 1 And Buffer.pVentasConsultar = 1 And Buffer.pVentasEditar = 1 And Buffer.pUsuarios = 1 Then
            Rc = DatosGotech.OLEDB.contactos_retornaTodo()
        Else
            Rc = DatosGotech.OLEDB.contactos_retornaTodo(Buffer.usuarioCodigo)
        End If
    ...
    Sub llenaLista(ByVal criterio As String)
        Dim Rc As ADODB.Recordset
        If Buffer.pClientes = 1 And Buffer.pSeguimientos = 1 And Buffer.pProductos = 1 And Buffer.pVentasRealizar = 1 And Buffer.pVentasConsultar = 1 And Buffer.pVentasEditar = 1 And Buffer.pUsuarios = 1 Then
            Rc = DatosGotech.OLEDB.contactos_retornaTodo_LIKE(criterio)
        Else
            Rc = DatosGotech.OLEDB.contactos_retornaTodo_LIKE(Buffer.usuarioCodigo, criterio)
        End If
        ListView1.Items.Clear()
        Do While Rc.EOF = False
            Dim t As New ListViewItem
    ...
    Sub seleccionaContacto()
        If ListView1.SelectedItems.Count >= 1 Then
            Buffer.contactosCodigo = CInt(ListView1.SelectedItems(0).Text)
            Buffer.contactosNombre = ListView1.SelectedItems(0).SubItems(1).Text
            Me.Parent.Controls("textBox2").Text = "  " + Buffer.contactosCodigo.ToString() + " .- " + Buffer.contactosNombre
